/*  Buscador de nombres: 
    Crea una función que reciba por parámetro un array y el valor 
    que desea comprobar que existe dentro de dicho array - comprueba 
    si existe el elemento, en caso que existan nos devuelve un true 
    y la posición de dicho elemento y por la contra un false. */

const nameFinder = [
  'Peter',
  'Steve',
  'Tony',
  'Natasha',
  'Clint',
  'Logan',
  'Xabier',
  'Bruce',
  'Peggy',
  'Jessica',
  'Marc'
];


function finderName(array, name) {
    const index = array.indexOf(name);
    return index !== -1?  { exists:true, index} : {exists: false};
}
      

console.log(finderName(nameFinder, "Jessica"));  // { exists: true, index: 9 }
console.log(finderName(nameFinder, "Popochas")); // { exists: false }